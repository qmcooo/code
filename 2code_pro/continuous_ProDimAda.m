function power=continuous_ProDimAda(n,a, lambdaw,h,p1)
s=1000;
% s=1;
p2=1;
c1=0.5*log(n)/sqrt(n);
...
%     y=sum(x,2)/sqrt(p1)+a*(sum(x,2)/sqrt(p1)).^2+error;

% study2
% y=x(:,1)+a*x(:,2).^3+error;%p1=3
% y=(x(:,1)+x(:,2))./sqrt(2)+a*((x(:,3)+x(:,4))./sqrt(2)).^3+error;%p1=4

% study3
% y=sum(x(:,1:(p1/2)),2)/sqrt(p1/2)+a*(sum(x(:,(p1/2+1):p1),2)/sqrt(p1/2)).^2+error;

% study4
% y=0.25*exp(2*x(:,1))+a*(x(:,2)).^3+sqrt(0.5)*error;%H41
% y=0.25*exp(2*x(:,1))+a*(x(:,2).^3+cos(pi*x(:,3))+x(:,4))+sqrt(0.5)*error;%H42
% y=0.25*exp(2*x(:,1))+a*(x(:,2).^3+cos(pi*x(:,3))+x(:,4)+abs(x(:,5))+abs(x(:,6)).^1.5+x(:,7).*x(:,8))+sqrt(0.5)*error;%H43
